...
module.exports = function(Napchart) {
  ...
  function move(e, chart) {
    ...
    if (chart.listeningForPenMove) {
      ...
      var scratchDistance = helpers.duration(
        initialPenLocation,
        currentLocation
      );
  ...
  function update(chart) {
    ...
    function move(hit, info) {
      // clone our element
      var originElement = chart.data.elements.find(
        element => element.id == hit.elementId
      );
      var changes = {
      ...
      if (hit.type == "start" || hit.type == "end") {
        ...
        var collision = isCollisionStartEnd(
          chart,
          start,
          end,
          originElement.lane,
        ...
        if (!laneIsLocked) {
          ...
          var closestLaneToCursor = lanes.reduce(
            (bestLane, thisLane, laneIndex) => {
              // distance to lane
              const distanceToLane = Math.abs(
                distance - (thisLane.start + thisLane.end) / 2
              );
        ...
        // move the bastard
        originElement.duration = helpers.duration(
          originElement.start,
          originElement.end
        );
        ...
        var start = snap(
          helpers.limit(Math.round(info.minutes - positionInElement))
        );
        ...
        if (laneIsLocked) {
          ...
          var elementsWithThisLane = chart.data.elements
            .filter(e => e.lane == originElement.lane)
            .map(e => e.id);
          chart.updateManyElements(relativeChanges, elementsWithThisLane);

          requestAnimationFrame(chart.onUpdate)
        } else {
  ...
  function isCollision(chart, start, end, lane, id) {
    ...
    var found = chart.data.elements.find(element => {
      ...
      return (
        (helpers.isInside(element.start + 1, start, end) ||
  ...
  function isCollisionStartEnd(chart, start, end, lane, id, dragStartOrEnd) {
    ...
    if (!collides) {
      ...
      var closestElement = chart.data.elements.reduce(
        (bestElement, thisElement) => {
          ...
          var distance = helpers.range(
            ownElement[stationaryStartOrEnd],
            thisElement[stationaryStartOrEnd],
            anticlockwise
          );
