...
module.exports = function (Napchart) {
  ...
  function move (e, chart) {
    ...
    if (chart.listeningForPenMove) {
      ...
      // the reason for 1435 is that -5 = 1435
      if (scratchDistance > 5 && scratchDistance < 1435) {
  ...
  function update (chart) {
    ...
    function move (hit, info) {
      ...
      if (hit.type == 'start' || hit.type == 'end') {
        ...
        window.chart = chart
        // write changes (if it has changed)
  ...
  function isInsideALane (distance, lanes) {
    return lanes.findIndex(lane => (distance > lane.start && distance < lane.end))
  }
}
